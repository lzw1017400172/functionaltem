...
import com.deehow.core.support.Token;
...
public final class WebUtil {
    ...
    /** 保存当前租户 */
    public static final void saveCurrentTenant(Object tenant) {
        setSession(Constants.CURRENT_TENANT, tenant);
    }
    ...
    /** 保存当前租户 */
    public static final void saveCurrentTenant(HttpServletRequest request, Object tenant) {
        setSession(request, Constants.CURRENT_TENANT, tenant);
    }
    ...
    /** 获取当前租户 */
    public static final Long getCurrentTenant() {
        Subject currentUser = SecurityUtils.getSubject();
        if (null != currentUser) {
            try {
                Session session = currentUser.getSession();
                if (null != session) {
                    return Long.valueOf(Objects.toString(session.getAttribute(Constants.CURRENT_TENANT),"-1"));
                }
            } catch (InvalidSessionException e) {
                logger.error(e);
            }
    ...
    /** 获取当前租户 */
    public static final Object getCurrentTenant(HttpServletRequest request) {
        try {
            HttpSession session = request.getSession();
            if (null != session) { 
                return session.getAttribute(Constants.CURRENT_TENANT);
            }
        } catch (InvalidSessionException e) {
            logger.error(e);
        }
